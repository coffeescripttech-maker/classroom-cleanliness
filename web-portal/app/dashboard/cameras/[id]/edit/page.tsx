  ...
  const fetchCamera = async () => {
    try {
      ...
      if (result.success && result.data) {
        ...
        setFormData({
          classroom_id: camera.classroom_id?.toString() || '',
          name: camera.name || '',
          ip_address: camera.ip_address || '',
          port: camera.port || 554,
          username: camera.username || 'admin',
          password: camera.password || '',
          rtsp_path: camera.rtsp_path || '/cam/realmonitor?channel=1&subtype=0',
          status: camera.status || 'active'
        });
      }
    } catch (error) {
      console.error('Failed to fetch camera:', error);
      alert('Failed to load camera details');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        alert(result.error || 'Failed to update camera');
      }
    } catch (error) {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-1">Update camera configuration</p>
      </div>
